...
function is31Flavors(targetArray){
   if (targetArray.length === 31){
       return true;
   } else {
       return false;
   }
...
function getFlavorByIndex(targetArray, number){
    return targetArray[number];
}
...
function removeFlavorByName(targetArray, flavor){
    for (let i = 0; i < targetArray.length; i++){
        if (targetArray[i] === flavor){
...
function filterByWord(targetArray, flavor){
    let newArray = [];
    for (let i = 0; i < targetArray.length; i++){
        if (targetArray[i].includes(flavor)){
